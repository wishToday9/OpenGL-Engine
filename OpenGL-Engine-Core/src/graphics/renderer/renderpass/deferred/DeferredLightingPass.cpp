...
namespace OpenGL_Engine {
	...
	DeferredLightingPass::DeferredLightingPass(Scene3D* scene)
	...
	DeferredLightingPass::DeferredLightingPass(Scene3D* scene, FrameBuffer* framebuffer)
		:RenderPass(scene, RenderPassType::LightingPassType), m_AllocatedFramebuffer(false), m_Framebuffer(framebuffer)
	{
		m_LightingShader = ShaderLoader::loadShader("src/shaders/deferred/pbr_lighting_pass.vert", "src/shaders/deferred/pbr_lighting_pass.frag");
	}
	...
	OpenGL_Engine::LightingPassOutput DeferredLightingPass::executeRenderPass(ShadowmapPassOutput& shadowmapData, GeometryPassOutput& geometryData, ICamera* camera, bool useIBL)
	{
