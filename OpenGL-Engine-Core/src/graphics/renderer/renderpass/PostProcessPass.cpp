...
namespace OpenGL_Engine {
	...
	PostProcessPass::PostProcessPass(Scene3D* scene) 
		: RenderPass(scene), m_SsaoRenderTarget((unsigned int)(Window::getRenderResolutionWidth() * 0.5f), (unsigned int)(Window::getRenderResolutionHeight() * 0.5f), false), m_SsaoBlurRenderTarget((unsigned int)(Window::getRenderResolutionWidth() * 0.5f), (unsigned int)(Window::getRenderResolutionHeight() * 0.5f), false),
		m_TonemappedNonLinearTarget(Window::getWidth(), Window::getHeight(), false), 
		m_ScreenRenderTarget(Window::getWidth(), Window::getHeight(), false),
		m_ResolveRenderTarget(Window::getRenderResolutionWidth(), Window::getRenderResolutionHeight(), false),
		m_BrightPassRenderTarget(Window::getWidth(), Window::getHeight(), false),
		m_BloomFullRenderTarget(Window::getWidth(), Window::getHeight(), false),
		m_BloomHalfRenderTarget((unsigned int)(Window::getWidth() * 0.5f), (unsigned int)(Window::getHeight() * 0.5f), false),
		m_BloomQuarterRenderTarget((unsigned int)(Window::getWidth() * 0.25f), (unsigned int)(Window::getHeight() * 0.25f), false),
		m_BloomEightRenderTarget((unsigned int)(Window::getWidth() * 0.125f), (unsigned int)(Window::getHeight() * 0.125f), false),
		m_FullRenderTarget(Window::getWidth(), Window::getHeight(), false),
		m_HalfRenderTarget((unsigned int)(Window::getWidth() * 0.5f), (unsigned int)(Window::getHeight() * 0.5f), false), 
		m_QuarterRenderTarget((unsigned int)(Window::getWidth() * 0.25f), (unsigned int)(Window::getWidth() * 0.25f), false),
		m_EightRenderTarget((unsigned int)(Window::getWidth() * 0.125f), (unsigned int)(Window::getHeight() * 0.125f), false),
		m_SsaoNoiseTexture(), m_ProfilingTimer(), m_EffectsTimer()
	{
		...
		DebugPane::bindBloomEnabled(&m_BloomEnabled);
	...
	PreLightingPassOutput PostProcessPass::executePreLightingPass(GeometryPassOutput& geometryData, ICamera* camera)
	{
		...
		m_SsaoShader->setUniform("numKernelSamples", SSAO_KERNEL_SIZE);
	...
	void PostProcessPass::executePostProcessPass(Framebuffer* framebufferToProcess) {
		...
		if (inputFramebuffer->getWidth() != m_ScreenRenderTarget.getWidth() 
			|| inputFramebuffer->getHeight() != m_ScreenRenderTarget.getHeight()) 
		{
		...
		if (m_ChromaticAberrationEnabled) {
			if (framebufferToRenderTo == &m_FullRenderTarget) {
				framebufferToRenderTo = &m_TonemappedNonLinearTarget;
			}
			else {
...
#endif
		//finally render scene to the window's framebuffer
		Window::bind();
		Window::clear();


		GLCache::getInstance()->switchShader(m_PassthroughShader);
	...
	void PostProcessPass::tonemapGammaCorrect(Framebuffer* target, Texture* hdrTexture)
	{
		...
		m_GLCache->setFaceCull(true);
		m_GLCache->setCullFace(GL_BACK);
		m_GLCache->setStencilWriteMask(false);
	...
	void PostProcessPass::vignette(Framebuffer* target, Texture* texture, Texture* optionalVignetteMask /*= nullptr*/)
	{
		glViewport(0, 0, target->getWidth(), target->getHeight());
		...
		m_VignetteShader->setUniform("color", m_VignetteColour);
	...
	OpenGL_Engine::Texture* PostProcessPass::bloom(Texture* hdrSceneTexture)
	{
		m_GLCache->setDepthTest(false);
		m_GLCache->setBlend(false);
		m_GLCache->setFaceCull(true);
