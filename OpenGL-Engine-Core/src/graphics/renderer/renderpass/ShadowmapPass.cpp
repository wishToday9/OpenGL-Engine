...
namespace OpenGL_Engine {
	...
	ShadowmapPass::ShadowmapPass(Scene3D* scene) : RenderPass(scene), m_AllocateFramebuffer(true)
	{
	...
	ShadowmapPass::ShadowmapPass(Scene3D* scene, Framebuffer* customFramebuffer) : RenderPass(scene), m_AllocateFramebuffer(false), m_ShadowmapFramebuffer(customFramebuffer)
	{
		m_ShadowmapShader = ShaderLoader::loadShader("src/shaders/shadowmap.vert", "src/shaders/shadowmap.frag");
	}
