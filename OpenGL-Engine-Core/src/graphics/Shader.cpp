...
namespace OpenGL_Engine { namespace graphics {
	...
	Shader::Shader(const char *vertPath, const char *fragPath)
		: m_VertPath(vertPath), m_FragPath(fragPath), m_GeoPath("")
	{
		m_ShaderID = load();
	}

	Shader::Shader(const char* vertPath, const char* fragPath, const char* geoPath)
		: m_VertPath(vertPath), m_FragPath(fragPath), m_GeoPath(geoPath)
	{
		m_ShaderID = load();
	}
	...
	GLuint Shader::load() {
		...
		if (m_GeoPath != "") {
			...
			std::string geomSourceString = FileUtils::readFile(m_GeoPath);
